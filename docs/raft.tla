-------------------------------- MODULE raft --------------------------------
(* This is the formal specification for the Raft replication algorithm. *)

EXTENDS Naturals, FiniteSets, Sequences, TLC, TLAPS
...
(* A bag of records representing requests and responses set from one server to another.  *)
(* TLAPS doesn't support the Bags module, so this is a function mapping Message to PNat. *)
VARIABLE messages

(* A history variable used in the proof. This would not be present in an implementation.           *)
(* Keeps track of successful elections, including the initial logs of the leader and voters' logs. *)
VARIABLE elections

(* A history variable used in the proof. This would not be present in an implementation. *)
(* Keeps track of every log ever in the system.                                          *)
VARIABLE allLogs

----
(* The following variables are all per server (functions with the domain Replica). *)
...
VARIABLE log
(* The latest entry the state machine may apply is commitIndex. *)
VARIABLE commitIndex
logVars == <<log, commitIndex>>

(* The following variables are used only on candidates: *)
(* The set of servers from which the candidate has received a RequestVote response in this term. *)
VARIABLE votesResponded
(* The set of servers from which the candidate has received a vote in this term. *)
VARIABLE votesGranted
(* A history variable used in the proof. This would not be present in an implementation. *)
(* Keeps track of the log of each voter.                                                 *)
VARIABLE voterLog
...
VARIABLE lastAgreeIndex
leaderVars == <<nextIndex, lastAgreeIndex, elections>>

(* End of per server variables. *)
----

(* All variables; used for stuttering. *)
...
(* The recipient is done processing the message. *)
...
(* The values nextIndex[i][i] and lastAgreeIndex[i][i] are never read, since    *)
(* the leader does not send itself messages. It's still easier to include these *)
(* in the functions. *)
...
DuplicateMessage == /\ \E m \in DOMAIN messages :
                           Send(m)
                    /\ UNCHANGED <<replicaVars, candidateVars, leaderVars, logVars>>

(* The network drops a message *)
DropMessage == /\ \E m \in DOMAIN(messages) :
                      Discard(m)
               /\ UNCHANGED <<replicaVars, candidateVars, leaderVars, logVars>>
...
Restart(i) == /\ UNCHANGED <<messages, currentTerm, votedFor, log, elections>>
...
BecomeLeader(i) == /\ state[i] = Candidate
                   ...
                   /\ nextIndex'      = [nextIndex EXCEPT ![i] =
                                            [j \in Replica |-> Len(log[i]) + 1]]
                   /\ lastAgreeIndex' = [lastAgreeIndex EXCEPT ![i] =
                                            [j \in Replica |-> 0]]
                   /\ elections' = elections \cup
                                        {[e.eterm     |-> currentTerm[i],
                                          e.eleader   |-> i,
                                          e.elog      |-> log[i],
                                          e.evotes    |-> votesGranted[i],
                                          e.evoterLog |-> voterLog[i]]}
...
ClientRequest(i, v) == /\ state[i] = Leader
                       ...
                       /\ UNCHANGED <<messages, replicaVars, candidateVars, leaderVars,
                                      commitIndex>>
...
(* Server i receives an AppendEntries request from server j with m.mterm <= currentTerm[i]. *)
(* This just handles m.entries of length 0 or 1, but implementations could safely accept    *)
(* more by treating them the same as multiple independent requests of 1 entry.              *)
...
HandleAppendEntriesResponse(i, j, m) ==
    ...
    /\ \/ /\ m.mlastAgreeIndex > 0
          ...
          /\ LET (* Agree(index) is the set of replicas that agree up through index. *)
                 Agree(index) == {i} \cup {k \in Replica : lastAgreeIndex'[i][k] >= index}
                 (* The set of indexes for which a quorum agrees *)
       ...
       \/ /\ m.mlastAgreeIndex = 0
          ...
          /\ UNCHANGED <<lastAgreeIndex, commitIndex>>
    /\ Discard(m)
